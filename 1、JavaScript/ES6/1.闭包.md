...
# (2) hooks的设计本质：闭包驱动
1. useState：渲染快照 + 闭包导致的“旧值”现象
```jsx
const [count, setCount] = useState(0);
```
组件重新渲染时，`useState(0)` 并不会每次都把 count 重新初始化为 0；React 会把 state 存在组件对应的内部结构里（例如 Fiber 上），每次渲染只是**读取当前 state 并返回“本次渲染的快照值”**。闭包相关的点在于：你在本次渲染里创建的事件处理函数/回调函数，会闭包捕获本次渲染的 `count`（快照），因此可能出现“旧值/陈旧闭包”现象。

2. useEffect：回调闭包捕获变量；依赖数组用于决定是否需要“换一份新回调”
```jsx
useEffect(() => {
  console.log(count); // 闭包捕获当次渲染的 count
}, [count]);
```
`useEffect` 的回调函数是在本次渲染创建的，因此它会闭包捕获本次渲染的 `count`。依赖数组 `[count]` 本身不是“被闭包捕获”，而是 React 在每次渲染时拿到的一个**依赖快照**：React 会把它和上一次的依赖做比较（逐项 `Object.is`），只有依赖变化时才会重新安排 effect，并让新的回调（新的闭包）在之后某个时机执行。若依赖不变，effect 不会重新执行，你看到的就还是旧回调里捕获的旧值。
...
```
`useCallback/useMemo` 会在依赖不变时复用上一次返回的函数/值（也就是复用上一次创建的那份闭包）；依赖变化时才创建新的函数/值（新的闭包），从而避免不必要的函数创建或昂贵计算。再配合 `React.memo` / `useMemo` 的引用稳定性，可以减少子组件因 props 引用变化导致的重渲染。

一句话：Hook API 让你把“函数”交给 React 保存并稍后调用；这些函数天然会闭包捕获当次渲染的变量。
渲染一次 = 产生一套新的局部变量
你交给 Hook 的回调函数 = 会记住（闭包）它来自哪次渲染
deps = 告诉 React 什么时候该换一份新的回调（新的闭包）
