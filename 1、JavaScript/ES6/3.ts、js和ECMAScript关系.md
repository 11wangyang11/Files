...
// TS 代码：class + 参数属性
// 下面演示两种写法：
// 1) 原型方法 greet()：挂在 Person.prototype 上（ES6 class 的“方法”语法）
// 2) 类字段 greet = () => {}：每个实例一份（class fields，不是 ES6 class 本身的一部分）
class Person {
  ...
  // 写法 2：类字段 + 箭头函数（每个实例一份，this 自动绑定）
  greetArrow = () => {
    console.log(`Hello, my name is ${this.name}`);
  };
}
...
编译后的 JavaScript 代码（假设 target 为 ES6）可能如下所示：
```js
class Person {
  constructor(name) {
    this.name = name;
    // 类字段初始化通常会下沉到 constructor
    this.greetArrow = () => {
      console.log(`Hello, my name is ${this.name}`);
    };
  }

  // 原型方法会保留在 class body 中（等价于 Person.prototype.greet）
  greet() {
    console.log(`Hello, my name is ${this.name}`);
  }
}
...
补充：
- `greet()`：在 `Person.prototype` 上，所有实例共享同一个函数；
- `greetArrow`：在 constructor 里赋值，每个实例都会创建一份函数（不在 `Person.prototype` 上），好处是 this 自动绑定，代价是实例多时更耗内存。

#### 原型方法 vs 箭头函数类字段（this 与性能差异）
1. this 绑定不一样：
- 原型方法：this 由“调用方式”决定，方法被单独拿出来调用可能丢失 this；
- 箭头函数类字段：this 词法绑定，创建时就固定为当前实例（常用于回调/事件）。
...
const a = new A("Alice");
const f1 = a.greet;
const f2 = a.greetArrow;

f1(); // 可能 undefined / 报错（严格模式下 this 为 undefined）
f2(); // "Alice"
```

2. 存放位置不一样（是否共享）：
- 原型方法在 `A.prototype` 上：所有实例共享一份函数；
- 箭头函数类字段在实例上：每个实例一份函数（更耗内存，但 this 更省心）。
