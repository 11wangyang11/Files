...
4. 默认绑定：在严格模式下为`undefined`，非严格模式下为全局对象(浏览器为`window`)。
...
```text
  ...
  └── __proto__ → Child.prototype
                   （上面有：childInstanceMethod）
                          │
                          └── __proto__ → Parent.prototype
                                             （上面有：parentInstanceMethod）
                                                    │
...
// 依次打印：alice → Person.prototype → Object.prototype
// 下一轮 getPrototypeOf(Object.prototype) 得到 null，循环结束
// 用「点属性」查不到链的终点，属性查找一直找到 Object.prototype 还没有 → 返回 undefined。
```
...
# (6) 构造函数
前面原型链中可以看出，constructor 是原型对象（prototype）上的一个属性，**默认**指向创建该原型对象的构造函数（例如 `Foo.prototype.constructor === Foo`）。那在 JavaScript 中，为什么有了 `__proto__`（实例到原型的“前向链”），还需要 constructor（原型到构造函数的“反向指针”）？

首先，原型对象知道自己的构造函数，这是原型的自我描述 / 反射能力（调试、框架元编程时经常用到）。但要注意：constructor **不是强约束**，手动替换 prototype 或修改原型链时，它可能会丢失或被改写。

其次，一些内置/库会读取 `constructor` 相关信息来决定“派生对象”用什么构造器创建（典型是 `Symbol.species` 机制），但这不是“可靠类型标识”。因此不推荐用 `constructor` 来做严肃的类型判断（它可被覆盖、跨 iframe/realm 也会不一致）。
```js
const arr = [1, 2, 3];
...
// 例：部分方法会用 species 决定返回的构造器（不是类型判断）
class MyArray extends Array {
  static get [Symbol.species]() {
    return Array; // 让 map 等方法返回普通 Array
  }
}
...
```
最后，ES6 的 class 本质仍是构造函数和原型链：实例方法放在 `Foo.prototype` 上，静态方法放在 `Foo` 上；而 `constructor` 作为“原型到构造函数”的约定入口，在语言生态里长期存在。总之，尽管日常开发不一定直接用 constructor，但它在调试/反射与部分派生构造逻辑中依然有价值。
...
首先，Function 是最底层的，Object 也是 Function 构造的。其次，prototype 和__proto__其实都是对象。我们说原型链的底层都是 Object.prototype，这一点可能大家会奇怪，毕竟 Object 也是 Function 构建的，底层不应该是 Function.prototype 吗？其实，Object.**proto===Function.prototype，这一点是正确的，也符合规定。但是，Function.prototype 本身也是对象**，所以Function.prototype.__proto__===Object.prototype，最终还是指向 Object.prototype。所以，这个底层设计是合理的。
...
1. 基本类型判断使用 typeof，适用于 number、string、boolean、undefined、symbol、bigint 和 function。但无法区分“对象类型”（包含数组、普通对象、以及 null 等，都会返回 "object"）；
2. null 单独使用 `x === null` 判断；
3. 数组优先使用 `Array.isArray(x)`（可靠）。使用 `instanceof Array` 可能存在跨环境（跨 iframe/realm）问题；
4. 对象类型判断通常有两类思路：
   - 内置对象（Map、Set、Date、RegExp 等）可使用 `Object.prototype.toString.call(x)`；
   - 自定义对象/类实例可使用 `instanceof`，但同样要注意跨环境（跨 realm）问题。

```js
Object.prototype.toString.call(new Map()); // "[object Map]"
Object.prototype.toString.call(new Map()).slice(8, -1).toLowerCase(); // "map"
```
...
当调用`Object.prototype.toString.call(obj)`时，引擎会检查 obj 的内部标识，并返回[object Type]格式的字符串。
...
4. **直接调用 toString**：前面提到，原型链最后指向的都是`Object.prototype`，所以从原型链的角度来看，对象是可以直接调用 toString 方法的，往上查找最终可以查到`Object.prototype.toString`。但是，很多对象的 toString 方法都被覆盖(override)了，比如数组的 toString 方法会转换为字符串内容。如下：
