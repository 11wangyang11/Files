...
class CountdownTimer extends React.PureComponent<CountDownTimerProps, CountDownTimerState> {
    private endTime: number = 0;
    private timer?: ReturnType<typeof setInterval>;

    constructor(props: CountDownTimerProps) {
        super(props);
        this.state = { timeLeft: '00:00' };
    }

    componentDidMount() {
        this.start(this.props.leftSeconds);
    }

    componentDidUpdate(prevProps: CountDownTimerProps) {
        if (prevProps.leftSeconds !== this.props.leftSeconds) {
            this.start(this.props.leftSeconds);
        }
    }
    ...
    private start = (leftSeconds: number) => {
        if (this.timer) {
            clearInterval(this.timer);
        }
        this.endTime = Date.now() + leftSeconds * 1000;
        this.updateTimer(); // 立即执行一次，确保初始显示正确
        this.timer = setInterval(this.updateTimer, 100); // 使用更短间隔提升精度
    };
    ...
    private updateTimer = () => {
        ...
        if (display !== this.state.timeLeft) {
            this.setState({ timeLeft: display });
        }
        ...
        if (remaining <= 0) {
            ...
            this.props.onCountdownEndCallback();
        }
    };
    ...
    render() {
        return (
            <View>
                <HText>{this.state.timeLeft}</HText>
            </View>
        );
    }
}
