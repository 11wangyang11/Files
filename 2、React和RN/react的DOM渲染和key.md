...
# 1、没有 key 的情况
当元素列表没有**显式声明 key 时**，React 默认使用 索引（index） 作为隐式 key。此时对比逻辑如下：  
...
# 2、有 key 的情况
当显式声明**唯一且稳定**的 key 时，React 会基于 key 进行匹配：

1. 跨位置匹配：React 会通过 key 匹配新旧列表中的相同元素，无论它们的位置是否变化。
2. 复用实例：匹配到相同 key 的元素会直接复用组件实例，避免不必要的销毁和重建。
3. 高效移动：仅对无法匹配的元素进行新增或删除，已有元素只需移动位置。**加入Key后，React 会对比新旧列表，计算最长上升子序列（LIS），只移动必要的节点。减少移动次数是仅次于节点复用的一种优化。当使用默认 index 作为 key 时,React 的 Diff 算法会进入“简单模式”。在这个模式下,它不会执行基于 Map 的 LIS 算法,而是执行一种更简单的“逐位对比”策略。**
