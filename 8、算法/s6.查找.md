...
function insertSearch(arr, target) {
    ...
    while(left <= right) {
        // 边界值检查，如果不存在，直接结束了（插值可能会超出左右边界，比如大于arr[x]但小于arr[x+1]，这样边界移动一位，不在arr内，直接返回* -1。否则可能mid会进入死循环。比如arr = [1, 100, 200, 300], target = 150，前几轮后：left = 2, right = 3，计算mid又变回1，然后left=mid+1=2
